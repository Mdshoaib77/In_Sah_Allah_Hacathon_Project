        ...
        {tutors.slice(0, 5).map((tutor, i) => (
          ...
          >
            ...
            <div className="flex flex-col w-full gap-2 p-4">
              <button onClick={()=> {navigate(`/session/${tutor._id}`), scrollTo(0,0)}} className="btn-white !font-bold !text-xs !p-2">
              View Profile
              </button>
              <button onClick={()=> {navigate('/tutors'), scrollTo(0,0)}} className="btn-white !font-bold !text-xs !p-2">
              Explore Tutors
              </button>
            </div>
            </div>
