...
const Session = () => {
  ...
  const { tutors, currency, backendUrl, token, getTutorsData, navigate } = useContext(AppContext);
  ...
  useEffect(() => {
    ...
    const generateSlots = () => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        // For today, start at the next available 30-minute mark at least 1 hour later
        if (i === 0) {
            ...
            <div>
              <div className="flexBetween max-w-sm">
                ...
                <span className="relative pl-5 pr-2 ring-1 ring-slate-900/10 rounded-full">
                  <span className={`absolute top-1 left-1 h-2.5 w-2.5 rounded-full bg-green-500 ${!tutorInfo.available && "bg-red-500"}`} />
              ...
              <div className="mt-8">
                ...
                <button onClick={bookSession} className="btn-secondary !text-tertiary mt-4">
                  Book a Session
                </button>
              </div>
            </div>
