...
const AllSessions = ({ token }) => {
  const [orders, setOrders] = useState([]);
  const { sessions, setSessions, getAllSessions, aToken, cancelSession } =
    ...
    <div className="px-2 sm:px-8 py-12 h-screen sm:pl-[23%]">
      <div className="max-h-[80vh] min-h-[60vh] overflow-y-scroll">
        ...
        {sessions.reverse().map((session, index) => (
          ...
          >
            <h5 className="h5 sm:hidden md:flex">{index + 1}</h5>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className={`${!session.isCompleted ? "hidden" : "block"} disabled:cursor-not-allowed disabled:text-green-500 btn-ghost max-md:!px-1 !py-1 !text-xs !rounded`}
              >
                Completed
              </button>
            </div>
